...
rotation_matrix = np.array([[0.9999103,   0.01229562,  0.00531049],
                            [-0.01237053, 0.99982106, 0.01431169],
                            [-0.00513357, -0.0143761, 0.99988348]])

# translation_vector: 3x1 向量，将相机坐标系原点平移到末端的坐标
# 形式上表示 t_camera_to_ee，使得：p_ee = R_camera_to_ee * p_cam + t_camera_to_ee
# 注意单位：看起来该向量以米为单位（例如 -0.0239 m），因此使用时需要保证输入坐标单位一致
# translation_vector = np.array([-0.02391399, -0.07437914, -0.15656422]) # 原始 
# 实际测试的Z轴相比正确值都低10mm，这里修正了一下减了0.01m
translation_vector = np.array([-0.02391399, -0.07437914, -0.16656422]) # 实际测试修正值
